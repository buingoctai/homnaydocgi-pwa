...
const RecommendAudio = (props: Props) => {
  ...
  const addAddAudio = useCallback(
    (_idx: number, url: string) => {
      const newres = {
        ...response,
        recommendList: response['recommendList'].filter((r, idx) => idx !== _idx),
      };
      setResponse(newres);
  ...
  return (
    <div className="recommend-container">
      <span className="title">
        Tất cả audio liên quan{' '}
        <span style={{ fontStyle: 'italic', fontSize: '16px' }}>{`"${capitalizeFirstLetter(
          props.audioName
        )}"`}</span>
      </span>
      <div className="list">
        ...
        {response &&
          response['recommendList'].map((r, idx) => (
            <div className="recommend-item">
              <img src={r.thumb} width={60} height={40} />
              <span className="truncate-line title">{capitalizeFirstLetter(r.title)}</span>
              <span className="length">{secondsToHms(r.lengthSeconds)}</span>
              <Button
                text="Thêm"
                onClick={() => addAddAudio(idx, r.url)}
                style={{ fontSize: '12px' }}
              />
            </div>
          ))}
